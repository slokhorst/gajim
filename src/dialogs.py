...
class SingleMessageWindow:
	...
	message='', resource='', session=None, form_node=None, encrypted=None):
		...
		if encrypted is None:
			jid = to # cannot use self.get_contact(), as self.to_entry does not exist yet
			contact = None
			if '@' in jid:
				if '/' in jid:
			...
			if contact: # only load if we have a valid contact
				encrypted = gajim.config.get_per('contacts', contact.jid, 'gpg_enabled')
		if encrypted is None:
			encrypted = False
